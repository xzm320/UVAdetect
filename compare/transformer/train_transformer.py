...
from compare.transformer.transformer_model import TransformerClassifier, get_data_loaders
...
def train_transformer(args):
    ...
    best_val_acc = 0.0
    best_model_path = "compare/transformer/best_transformer.pth"
    ...
    for epoch in range(args.epochs):
        ...
        for signals, labels in train_pbar:
...
def save_training_curves(train_losses, val_losses, train_accs, val_accs, epochs):
    ...
    os.makedirs("compare/transformer/results", exist_ok=True)
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('compare/transformer/results/loss_curve.png', dpi=150)
    plt.close()
    
    print("[INFO] Loss曲线已保存至 compare/transformer/results/loss_curve.png")
...
def final_evaluation(model, val_loader, device):
    ...
    with torch.no_grad():
        for signals, labels in tqdm(val_loader, desc="Final Evaluation"):
            signals, labels = signals.to(device), labels.to(device)
            outputs = model(signals)
            _, predicted = torch.max(outputs, 1)
    ...
    # 保存报告
    os.makedirs("compare/transformer/results", exist_ok=True)
    with open("compare/transformer/results/classification_report.txt", "w", encoding="utf-8") as f:
        f.write("Transformer模型分类报告\n")
        f.write("="*40 + "\n\n")
        f.write(report)
    
    print("[INFO] 分类报告已保存至 compare/transformer/results/classification_report.txt")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="训练Transformer故障分类模型")
    ...
    # 开始训练
    model, best_acc = train_transformer(args)
    print(f"\n[FINAL] Transformer训练完成，最佳验证准确率: {best_acc:.2f}%")
